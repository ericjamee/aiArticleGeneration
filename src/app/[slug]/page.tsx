...
export default async function Post({ params }: { params: { slug: string } }) {
  ...
  const related = all.filter((p: any) => p.slug !== params.slug).slice(0,3);

  return (
    <main className="max-w-2xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold mb-2">{meta.title}</h1>
      <div className="text-xs text-gray-500 mb-8">{new Date(meta.date).toLocaleDateString()}</div>
      ...
      {related.length > 0 && (
        <section className="mt-10">
          ...
          <ul className="list-disc ml-6">
            {related.map((r: any) => (
              <li key={r.slug}><Link className="text-blue-600 hover:underline" href={`/${r.slug}`}>{r.title}</Link></li>
            ))}
          </ul>
        </section>
      )}
